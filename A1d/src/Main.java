...
public class Main
{
    ...
    public static void main(final String[] argv)
    {
		...
		//then get the shape to display
        shape = getShape(type, width, height);
        if (shape == null) {
          usage("failed to create shape");
        }
	...
	**/
                                 ...
                                 final int       height)
    {
		...
		try{
			shape = ((Shape)Class.forName(type).getDeclaredConstructor(args).
        ...
        catch(final IllegalAccessException ex)
        {
			...
			if (Objects.equals(ex.getCause().toString(), "BadWidthException")){
				usage("Bad Width");
				return null;
			}
		}
	...
	**/
    ...
    public static Displayer getDisplayer(final String name)
    {
        ...
        try
        {
            return ((Displayer)Class.forName(name).getDeclaredConstructor().newInstance());
        }
        catch(final ClassNotFoundException ex)
        {
